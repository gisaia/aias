...
class Settings(BaseModel, extra=Extra.allow):
    ...
    aeoprs_endpoint:str       | None = Field(title="ARLAS EO Product Registration Service endpoint.", examples=["http://127.0.0.1:8000"])
    ingesters:list[Driver]    | None = Field(title="Driver configuration")

class Configuration:
    settings:Settings         | None = Field(title="aeoprocesses Service configuration")
