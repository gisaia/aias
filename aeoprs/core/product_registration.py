...
import aeoprs.core.exceptions as exceptions
...
def upload_asset(collection:str, item_id:str, asset_name:str, file, content_type:str)->str:
...
def __delete_prefix(prefix:str):
    try:
        ...
        objects=s3.get_client().list_objects(Bucket=Configuration.settings.s3.bucket, Prefix=prefix)
...
def asset_exists(collection:str, item_id:str, asset_name:str)->bool:
    ...
    key=get_asset_relative_path(collection, item_id, asset_name)
    try:
        h=s3.get_client().head_object(Bucket=Configuration.settings.s3.bucket, Key=key)
        LOGGER.info(h)
        return True
    except Exception as e:
        LOGGER.debug(e)
...
def register_item(item:Item)->Item:
    ...
    if not __getES().indices.exists(index=__get_es_collection_name(item.collection)):
        ...
        if r.ok:
            ...
            LOGGER.error("Can not fetch the mapping for creating the ARLAS EO index. Aborting ...")
...
def __collect_bands(item:Item)->Item:
    ...
    for band in item.properties.eo__bands: bands[band.name]=band
    if item.assets is not None:
        for asset in item.assets.values():
            if asset.eo__bands is not None:
                for band in asset.eo__bands: bands[band.name]=band
...
def __add_generated_fields(item:Item)->Item:
    if item.centroid:
        item.properties.generated__geohash2=pgh.encode(item.centroid[1], item.centroid[0], precision=2)
        item.properties.generated__geohash3=pgh.encode(item.centroid[1], item.centroid[0], precision=3)
        item.properties.generated__geohash4=pgh.encode(item.centroid[1], item.centroid[0], precision=4)
        item.properties.generated__geohash5=pgh.encode(item.centroid[1], item.centroid[0], precision=5)
    if item.assets is not None:
        for asset in item.assets.values():
    ...
    item.properties.generated__minute_of_day=acquisition.hour*60 +acquisition.minute
...
def __set_assets_links(item:Item)->Item:
    for asset_name in item.assets:
        ...
        if asset.aeo__managed is True:
            ...
            object_relative_path=get_asset_relative_path(item.collection, item.id, asset_name)
            asset.name=asset_name
            asset.href=Configuration.settings.s3.asset_http_endpoint_url.format(Configuration.settings.s3.bucket, object_relative_path)
            asset.aeo__object_store_bucket=Configuration.settings.s3.bucket
    ...
    item.assets[Role.arlas_eo_item.value]=Asset(
            ...
            description="ARLAS EO item for {}".format(item.id),
            title="ARLAS EO item for {}".format(item.id),
            type="application/json",roles=[Role.arlas_eo_item.value]
        )
    item.assets[Role.arlas_eo_item.value].aeo__managed=True
    item.assets[Role.arlas_eo_item.value].aeo__object_store_bucket=Configuration.settings.s3.bucket
    ...
    for asset_name in item.assets:
        ...
        if asset.aeo__managed is True:
            if not asset_exists(item.collection, item.id, asset_name=asset_name):
                not_found.append(get_asset_relative_path(item.collection, item.id, asset_name))
    return not_found

def __check_register_item_params(item:Item):
    if item.collection is None or len(item.collection)==0:
        raise exceptions.InvalidItemsException([item.id], reason="Empty collection name")
    ...
    if item.properties.datetime is None and item.properties.start_datetime is None and item.properties.end_datetime is None:
        raise exceptions.InvalidItemsException([item.id], reason="item.properties.datetime or item.properties.start_datetime and item.properties.end_datetime must be set")
    if item.properties.datetime is None:
        item.properties.datetime=item.properties.start_datetime # Fall back date, necessary for ARLAS
