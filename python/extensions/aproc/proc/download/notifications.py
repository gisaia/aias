...
class Notifications:
    ...
    @staticmethod
    def init():
        LOGGER.info("SMTP configuration: {}".format(Configuration.settings.smtp.model_dump_json(exclude_none=True, exclude_unset=True)))
    ...
    def report(item: Item, subject: str, msg: str, to: list[str], context: dict[str, str], outcome: str = None):
        ...
        if (not subject and not msg) or not to or to == "anonymous":
            return
        try:
            if Configuration.settings.smtp.enabled:
