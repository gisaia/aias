    ...
    def transform_assets(self, url: str, assets: list[Asset]):
        if self.quicklook_path is None:
            if IngestDriver.must_build_preview(Driver.configuration, self.tif_path, local_remote_both="local"):
                ...
                geotiff_to_jpg(self.tif_path, Driver.OVERVIEW_SIZE, Driver.OVERVIEW_SIZE, output_path=quicklook.href, stretch=Driver.configuration.get('overview_stretch', False))
                ...
                with AccessManager.make_local(self.tif_path) as local_tif_path:
                    ...
                    geotiff_to_jpg(local_tif_path, Driver.OVERVIEW_SIZE, Driver.OVERVIEW_SIZE, output_path=quicklook.href, stretch=Driver.configuration.get('overview_stretch', False))
                    quicklook.size = AccessManager.get_size(quicklook.href)
                    self.quicklook_path = quicklook.href
                    assets.append(quicklook)
