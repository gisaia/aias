...
class Driver(IngestDriver):
    ...
    def transform_assets(self, url: str, assets: list[Asset]) -> list[Asset]:
        if IngestDriver.must_build_preview(Driver.configuration, self.tci_path, local_remote_both="local"):
            ...
            geotiff_to_jpg(self.tci_path, Driver.OVERVIEW_SIZE, Driver.OVERVIEW_SIZE, overview.href, [1, 2, 3], stretch=Driver.configuration.get('overview_stretch', False))
            ...
            with AccessManager.make_local(self.tci_path) as local_tci_path:
                ...
                geotiff_to_jpg(local_tci_path, Driver.OVERVIEW_SIZE, Driver.OVERVIEW_SIZE, overview.href, [1, 2, 3], stretch=Driver.configuration.get('overview_stretch', False))
                overview.size = AccessManager.get_size(overview.href)
                assets.append(overview)
        else:
