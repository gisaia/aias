...
class Driver(IngestDriver):
    ...
    def fetch_assets(self, url: str, assets: list[Asset]) -> list[Asset]:
        if self.quicklook_path:
            quicklook = ImageDriverHelper.make_local_preview_asset(self, url, self.quicklook_path, MimeType.JPG, AssetFormat.jpg)
    ...
    def transform_assets(self, url: str, assets: list[Asset]) -> list[Asset]:
        if self.quicklook_path is None and IngestDriver.must_build_preview(Driver.configuration, self.tif_path, local_remote_both="both"):
            ...
            geotiff_to_jpg(self.tif_path, Driver.OVERVIEW_SIZE, Driver.OVERVIEW_SIZE, quicklook.href, stretch=Driver.configuration.get('overview_stretch', False))
