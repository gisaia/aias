...
class Settings(BaseModel, extra='allow'):
    ...
    celery_result_backend_transport_options: dict = Field(default={}, title="Transport options", description="Celery's result backend transport options", examples="master_name: mymaster and sentinel_password: your-strong-password")
    processes: list[ProcessSettings] = Field(title="List of processes", description="List of APROC processes")
    airs_endpoint: str = Field(title="AIRS endpoint", description="ARLAS Item Registration Service endpoint")
